  ...
  return (
    <div className="product-row-db">
      ...
      <div style={{ display: "flex", gap: "5px" }}>
        <input type="number" placeholder="Qty" value={item.qty} onChange={(e) => updateItem(index, "qty", e.target.value)} style={{ flex: 1 }} />
        <input placeholder="Unit" value={item.unit} onChange={(e) => updateItem(index, "unit", e.target.value)} style={{ flex: 1 }} />
        <input type="number" placeholder="Rate" value={item.rate} readOnly className="input-readonly" style={{ flex: 1 }} />
      </div>
    </div>
  );
}